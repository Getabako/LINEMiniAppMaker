# LINE ミニアプリ Maker — Windows one-line installer & launcher
#
...
$InstallDir = if ($env:LINEMAKER_HOME)  { $env:LINEMAKER_HOME }  else { Join-Path $DesktopDir "LINEMiniAppMaker" }

function Info($msg) { Write-Host $msg -ForegroundColor Cyan }
function OK($msg)   { Write-Host $msg -ForegroundColor Green }
function Err($msg)  { Write-Host $msg -ForegroundColor Red }

Info "▶ LINE ミニアプリ Maker セットアップを開始します（Windows）"
